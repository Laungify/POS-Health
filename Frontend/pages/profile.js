...
function hasRoleInShops(shops, targetRole) {
  for (const shop of shops) {
    if (shop.roles && shop.roles.includes(targetRole)) {
      return true
    }
  }
  return false
...
function ListAvailability({ setFormState, edit }) {
  const { getUserId, staff, setStaff } = useAuthState()
  ...
  const fetchAvailability = async () => {
    try {
      ...
      const result = await API.get(`staff/${getUserId()}/availability`)
  ...
  const deleteAvailability = async id => {
    try {
      setLoading(true)
      await API.delete(`staff/${getUserId()}/availability/${id}`)
...
function AddAvailability({ setFormState }) {
  ...
  const saveAvailability = async () => {
    try {
      ...
      await API.post(`staff/${getUserId()}/availability`, {
        range: dateRange,
...
function EditAvailability({ setFormState, availability }) {
  const classes = useStyles()
  ...
  const updateAvailability = async () => {
    try {
      ...
      await API.patch(`staff/${getUserId()}/availability/${availability._id}`, {
        ...currentAvailability,
        range: dateRange,
      })
      setLoading(false)
...
function Availability() {
  ...
  return (
    <div>
      {formState === 'list' && (
        <ListAvailability setFormState={setFormState} edit={edit} />
      )}

      {formState === 'add' && <AddAvailability setFormState={setFormState} />}
...
export default function EditStaff() {
  const { staff, getUserId, setStaff } = useAuthState()
  ...
  const fetchProfile = async () => {
    ...
    try {
      const result = await API.get(`staff/${getUserId()}`)
      setStaff(result.data)
      setLoading(false)
    } catch (err) {
      setLoading(false)
  ...
  return !loading ? (
    <Layout>
      {hasRoleInShops(shops, 'doctor') ? (
        <EditDoctor staff={staff} />
      ) : (
        <EditDefault staff={staff} />
      )}
    </Layout>
  ) : null
}
